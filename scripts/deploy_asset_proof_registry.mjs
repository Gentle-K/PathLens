...
async function main() {
  ...
  if (!privateKey) {
    console.error(
      'ASSET_PROOF_REGISTRY_DEPLOYER_PRIVATE_KEY is required. The script also accepts PLAN_REGISTRY_DEPLOYER_PRIVATE_KEY, PRIVATE_KEY, or DEPLOYER_PRIVATE_KEY from .env.local.',
    )
    process.exit(1)
  }
  ...
  const { abi, bytecode } = compileAssetProofRegistry(solc)
  const network = resolveNetworkConfig()
  const initialAttester = process.env.ASSET_PROOF_REGISTRY_INITIAL_ATTESTER?.trim()
  ...
  console.log(`Deploying AssetProofRegistry to HashKey ${network.network}...`)
  ...
  if (initialAttester) {
    if (!viem.isAddress(initialAttester) || initialAttester === viem.zeroAddress) {
      console.error('ASSET_PROOF_REGISTRY_INITIAL_ATTESTER must be a non-zero EVM address.')
      process.exit(1)
    }

    const attesterTx = await walletClient.writeContract({
      address: receipt.contractAddress,
      abi,
      functionName: 'setAttester',
      args: [initialAttester, true],
      account,
    })
    await publicClient.waitForTransactionReceipt({ hash: attesterTx })
    console.log(`Initial attester granted: ${initialAttester}`)
    console.log(`Attester tx: ${network.explorerUrl}/tx/${attesterTx}`)
  }

  console.log('')
  console.log(
    network.network === 'mainnet'
      ? `Set HASHKEY_MAINNET_ASSET_PROOF_REGISTRY_ADDRESS=${receipt.contractAddress}`
      : `Set HASHKEY_TESTNET_ASSET_PROOF_REGISTRY_ADDRESS=${receipt.contractAddress}`,
  )
  console.log(`Optional legacy fallback: ASSET_PROOF_REGISTRY_ADDRESS=${receipt.contractAddress}`)
}
