...
describe('ReportPage', () => {
  ...
  it('renders the new RWA result sections without breaking the page', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Comparison matrix' }),
    ).toBeInTheDocument()
    expect(
      await screen.findByRole('heading', { name: 'Why this recommendation?' }),
    ).toBeInTheDocument()
    expect(
    ...
    expect(
      await screen.findByRole('heading', { name: 'Re-analysis diff' }),
    ).toBeInTheDocument()
